...
from zk3 import const
from zk3.attendance import Attendance
from zk3.exception import ZKErrorResponse, ZKNetworkError
from zk3.user import User
...
class ZK(object):
    ...
    def __create_header(self, command, command_string, checksum, session_id, reply_id):
        ...
        '''
        buf = pack('HHHH', command, checksum, session_id, reply_id) + command_string.encode()
    ...
    def __reverse_hex(self, hex):
        data = ''
        for i in reversed(list(range(len(hex) // 2))):
            data += hex[i * 2:(i * 2) + 2]
        return data
                ...
                if response == const.CMD_ACK_OK:
                    if userdata:
                        ...
                        while len(userdata) >= 72:
                            ...
                            privilege = int(privilege.hex(), 16)
                            password = password.decode(encoding_scheme)
                            password = password.strip('\x00|\x01\x10x')
                            group_id = group_id.decode(encoding_scheme)
                            group_id = group_id.strip('\x00|\x01\x10x')
                            user_id = user_id.decode(encoding_scheme)
                            user_id = user_id.strip('\x00|\x01\x10x')
                            name = name.decode(encoding_scheme)
                            name = name.strip('\x00|\x01\x10x')
    ...
    def get_attendance(self):
        ...
        if cmd_response.get('status'):
            if cmd_response.get('code') == const.CMD_PREPARE_DATA:
                ...
                while bytes > 0:
                    data_recv = self.__sock.recv(1032)
                    #data_recv = str(data_recv, 'ISO-8859-1')
                ...
                if response == const.CMD_ACK_OK:
                    if attendance_data:
                        ...
                        attendance_data = b''.join(attendance_data)
